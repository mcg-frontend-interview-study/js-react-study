...
Context는 prop drilling을 피하기 위한 하나의 수단이며 Context를 데이터를 하위 트리로 전달할 수 있게 된다.
...
Context 값이 변했을 때 context를 구독하는 컴포넌트만 리렌더링이 된다. 이 이유를 Context의 내부 동작을 살펴보면서 알아보도록 한다.
...
context라는 객체를 만들어서 Provider에 \_context 프로퍼티에 저장하고 동일하게 Consumer에도 저장한다.
...
workInProgress Fiber에는 변경될 값인 pendingProps, 이전 값인 memoizedProps 정보를 가지고 있으며 이전 값은 cookie이고 새로 변경된 값은 cookie changed이다.
...
파이버 스택에 새로운 값을 푸시하는 역할을 한다. 초기에 렌더링될 때 초기값인 `cookie`가 스택에 쌓이고, `cookie`가 `cookie changed`로 바뀌며 리렌더링이 될 때, 스택에 새로운 값이 푸시되어 스택의 상태는 `cookie`, `cookie changed` 상태가 되며 바뀐 상태를 바라보게 된다.
...
Object.is 메서드를 사용하여 oldValue와 newValue가 같은 값인지를 비교한 후 같으면 bailout을 호출하여 리렌더링을 하지 않도록 하고, 값이 다를 경우 consumer에게 상태가 변했음을 알리게 된다. 여기서는 `cookie`와 `cookie changed`값이 같지 않기 때문에 상태가 변했음을 알리는 propagateContextChange 함수가 호출된다.
